...
1. Run `python roadrunner.py next` to see the current task
2. Run `python roadrunner.py start TASK-XXX` before touching any files
3. Implement the task — stay within its scope
4. Run `python roadrunner.py validate TASK-XXX` to check your work
5. Fix any failures — validation commands are the source of truth, not your assessment
6. Run `python roadrunner.py complete TASK-XXX --notes "what you did"` to close the task
7. Run `python roadrunner.py reset TASK-XXX --summary "one line"` to write the boundary marker
...
```bash
python roadrunner.py block TASK-XXX --notes "reason for block"
```
...
A task is done when `python roadrunner.py validate TASK-XXX` exits 0.
...
```bash
python roadrunner.py status                        # show all task statuses
python roadrunner.py next                          # show next eligible task
python roadrunner.py start TASK-XXX               # begin a task
python roadrunner.py validate TASK-XXX            # run validation only
python roadrunner.py complete TASK-XXX --notes "" # complete with validation
python roadrunner.py block TASK-XXX --notes ""    # mark blocked
python roadrunner.py reset TASK-XXX --summary ""  # write boundary marker
python roadrunner.py health                        # system health check
```

...
1. Run `python roadmap_loop.py next` to see the current task
2. Run `python roadmap_loop.py start TASK-XXX` before touching any files
3. Implement the task — stay within its scope
4. Run `python roadmap_loop.py validate TASK-XXX` to check your work
5. Fix any failures — validation commands are the source of truth, not your assessment
6. Run `python roadmap_loop.py complete TASK-XXX --notes "what you did"` to close the task
7. Run `python roadmap_loop.py reset TASK-XXX --summary "one line"` to write the boundary marker
...
```bash
python roadmap_loop.py block TASK-XXX --notes "reason for block"
```
...
A task is done when `python roadmap_loop.py validate TASK-XXX` exits 0.
...
```bash
python roadmap_loop.py status                        # show all task statuses
python roadmap_loop.py next                          # show next eligible task
python roadmap_loop.py start TASK-XXX               # begin a task
python roadmap_loop.py validate TASK-XXX            # run validation only
python roadmap_loop.py complete TASK-XXX --notes "" # complete with validation
python roadmap_loop.py block TASK-XXX --notes ""    # mark blocked
python roadmap_loop.py reset TASK-XXX --summary ""  # write boundary marker
python roadmap_loop.py health                        # system health check
```
